...
| 2026-09-05 | Claude (sessão do plano) | `bash scripts/smoke-videos.sh --skip-multipart` | 71/71 ✔ — inclui a seção 8d da Fase 05 (slug público, stream/download anônimos, views, sugestões, 404 após despublicar) |
| 2026-09-05 | Claude (sessão do plano) | `bash scripts/smoke-videos.sh --skip-multipart` | 84/84 ✔ — inclui a seção 8e da Fase 06 (like único, comentário + resposta, profundidade, exclusão 403, auto-inscrição 409, inscrição, agregado social) |
| 2026-09-05 | Claude (sessão do plano) | `bash scripts/smoke-videos.sh --skip-multipart` | 89/89 ✔ — inclui a seção 8f da Fase 07 (feed, filtro por categoria, busca por título/canal, termo curto 400) |
| 2026-09-05 | Claude (sessão do plano) | `node scripts/browser-validation.mjs` | 23/23 ✔ — jornada completa: cadastro → upload → ready → player/seek → publicar → home/busca/watch anônimos → like, comentário, inscrição → /subscriptions → exclusão |
